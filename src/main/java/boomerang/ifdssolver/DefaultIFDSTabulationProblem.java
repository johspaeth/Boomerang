 ...
 * caches values that ought to be cached. This class uses the Factory Method
 * design pattern. The {@link InterproceduralCFG} is passed into the constructor
 * so that it can be conveniently reused for solving multiple different
 * {@link IFDSTabulationProblem}s. This class is specific to Soot.
 * 
 * @param <D>
...
public abstract class DefaultIFDSTabulationProblem<N, D, M, I extends BiDiInterproceduralCFG<N, M>>
	...
	public IPropagationController<N, D> propagationController() {
		return new IPropagationController<N, D>() {
			@Override
			public boolean continuePropagate(IPathEdge<N, D> edge) {
				return true;
			}
		};
	}
